    ...
    $('body').on('click', '.upload .submit', function (event){
        ...
        $.ajax({
            ...
            for (var i = 0; i < response.data.categories.length; i++) {
                $('.confirmation .form form .checkbox').html('<input type="checkbox" name="category-'+ response.data.categories[i].id +'" value="' + response.data.categories[i].id + '" /><label>' + response.data.categories[i].name + '</label>');
            }
